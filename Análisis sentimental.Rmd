...
```{r}
scores_chart_usuario<-sentimiento%>%
  inner_join(NRC_es, by=c("word"="word"))%>%
  filter(!grepl("positivo|negativo", sentiment))%>%
...
**genero cad usuario por separado **
scores_chart_try<-sentimiento%>%
  inner_join(NRC_es, by=c("word"="word"))%>%
  filter(!grepl("positivo|negativo", sentiment))%>%
  count(sentiment, Usuario)%>%
  filter(str_detect(Usuario, "the_runaddict"))

**y despues los juntos todos**
scores_full<-scores_chart_usuario%>%
  full_join(scores_chart_try, by=c("sentiment"="sentiment"))

Cambio el nombre de las columnas para que las leyendas se entienda = colnames(scores_full)<-c("sentiment", "aethos", "aethos_n")
